...
FILENAME="elasticsearch-${ELASTICSEARCH_VERSION}.tar.gz"
if [ ${ELASTICSEARCH_VERSION:0:1} -ge 7 ]
then
  FILENAME="elasticsearch-${ELASTICSEARCH_VERSION}-linux-x86_64.tar.gz"
  ELASTICSEARCH_DL_URL="https://artifacts.elastic.co/downloads/elasticsearch/${FILENAME}"
  ELASTICSEARCH_PLUGIN_BIN="${ELASTICSEARCH_DIR}/bin/elasticsearch-plugin"
elif [ ${ELASTICSEARCH_VERSION:0:1} -ge 5 ]
then
...
CACHED_DOWNLOAD="${HOME}/cache/${FILENAME}"
